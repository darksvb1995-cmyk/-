...
class DataProcessor:
    ...
    def load_id_data(self, folder_path, id_filename):
        ...
        try:
            ...
            for i in range(min(5, len(temp_df))):
                ...
                if any(keyword in row_str.lower() for keyword in ['шифр', 'конструктив', 'вид работ', 'объем', 'work']):
                    header_row = i
            ...
            for col in df.columns:
                ...
                # Шифр комплекта
                if ('шифр' in col_name or 'design' in col_name or 'документ' in col_name or
                    'комплект' in col_name or 'section' in col_name) and 'Шифр_комплекта' not in column_mapping.values():
                    column_mapping[col] = 'Шифр_комплекта'
                    self.log(f"  Найдена колонка для Шифр_комплекта: '{col}'")
                # Конструктивный элемент
                elif ('конструктив' in col_name or 'construction' in col_name or 'элемент' in col_name or
                      'element' in col_name) and 'Конструктивный_элемент' not in column_mapping.values():
                    column_mapping[col] = 'Конструктивный_элемент'
                    self.log(f"  Найдена колонка для Конструктивный_элемент: '{col}'")
                # Вид работ
                elif ('вид работ' in col_name or 'works' in col_name or 'work' in col_name or
                      'type of work' in col_name) and 'Вид_работ' not in column_mapping.values():
                    column_mapping[col] = 'Вид_работ'
                    self.log(f"  Найдена колонка для Вид_работ: '{col}'")
                # Объем по документу
                elif (('объем' in col_name or 'объём' in col_name or 'scope' in col_name or
                       'volume' in col_name or 'количество' in col_name or 'к-во' in col_name) and
                      ('по документу' in col_name or 'work scope' in col_name or 'work scopes' in col_name or
                       'документу' in col_name or 'document' in col_name or 'к исполнению' in col_name or
                       'выполнено' in col_name)) and 'Объем_по_документу' not in column_mapping.values():
                    column_mapping[col] = 'Объем_по_документу'
                    self.log(f"  Найдена колонка для Объем_по_документу: '{col}'")

            # Если не нашли колонку объема по ключевым словам, ищем любую числовую колонку
            if 'Объем_по_документу' not in column_mapping.values():
                self.log("Колонка 'Объем_по_документу' не найдена по ключевым словам, ищем числовые колонки...")
                for col in df.columns:
                    col_name = str(col).lower()
                    # Пропускаем колонки, которые явно не являются объемами
                    if any(exclude in col_name for exclude in ['id', 'номер', 'date', 'дата', 'код']):
                        continue

                    # Проверим, содержит ли колонка числовые данные
                    sample_data = df[col].dropna().head(10)
                    if len(sample_data) > 0:
                        # Пробуем преобразовать в числа
                        numeric_data = pd.to_numeric(sample_data, errors='coerce')
                        if numeric_data.notna().sum() > 0:  # Если есть хотя бы одно число
                            column_mapping[col] = 'Объем_по_документу'
                            self.log(f"  Найдена числовая колонка для Объем_по_документу: '{col}'")
                            break

            # Если все еще не нашли, попробуем найти первую числовую колонку
            if 'Объем_по_документу' not in column_mapping.values():
                self.log("Колонка 'Объем_по_документу' не найдена, ищем первую числовую колонку...")
                for col in df.columns:
                    sample_data = df[col].dropna().head(10)
                    if len(sample_data) > 0:
                        numeric_data = pd.to_numeric(sample_data, errors='coerce')
                        if numeric_data.notna().sum() > 0:
                            column_mapping[col] = 'Объем_по_документу'
                            self.log(f"  Найдена первая числовая колонка для Объем_по_документу: '{col}'")
                            break
            ...
            if 'Объем_по_документу' not in available_cols or len(available_cols) < 2:
    ...
    def optimize_matching(self, pivot_df, id_df):
        """Объединение данных по трём условиям - поиск по всему диапазону структуры"""
        self.check_cancelled()
        self.log("Объединение данных по трём условиям...")
        ...
        if missing_id_cols:
            ...
            self.log("Доступные колонки в ИД:")
        ...
        try:
            ...
            structure_work = pivot_df.copy()
            id_work = id_df.copy()

            # Очищаем и нормализуем данные для точного сравнения
            self.log("Подготовка данных для сопоставления...")
            self.update_progress(10, "Подготовка данных")

            # Очищаем строковые колонки в структуре
            for col in ['Проект', 'Конструкция', 'Вид_работ_по_классификатору']:
                if col in structure_work.columns:
                    structure_work[col] = structure_work[col].fillna('').astype(str).str.strip()

            # Очищаем строковые колонки в ИД
            for col in ['Шифр_комплекта', 'Конструктивный_элемент', 'Вид_работ']:
                if col in id_work.columns:
                    id_work[col] = id_work[col].fillna('').astype(str).str.strip()

            # Создаем временный столбец для накопления объемов из ИД
            structure_work['temp_volume'] = 0

            self.log("Поиск соответствий по всему диапазону структуры...")
            total_id_rows = len(id_work)
            matched_combinations = 0

            # Для каждой строки в ИД ищем все соответствующие строки в структуре
            for id_idx, (_, id_row) in enumerate(id_work.iterrows()):
                self.check_cancelled()

                # Обновляем прогресс
                if id_idx % 50 == 0:
                    progress = 20 + (id_idx / total_id_rows) * 70
                    self.update_progress(progress, f"Обработка ИД: {id_idx}/{total_id_rows}")

                id_shifr = id_row.get('Шифр_комплекта', '').strip()
                id_construction = id_row.get('Конструктивный_элемент', '').strip()
                id_work_type = id_row.get('Вид_работ', '').strip()
                id_volume = id_row.get('Объем_по_документу', 0)

                # Пропускаем строки с пустыми ключевыми полями
                if not id_shifr or not id_construction or not id_work_type:
                    continue

                # Ищем все строки в структуре, где совпадают все три параметра
                mask = (
                        (structure_work['Проект'] == id_shifr) &
                        (structure_work['Конструкция'] == id_construction) &
                        (structure_work['Вид_работ_по_классификатору'] == id_work_type)
                )

                matching_rows = structure_work[mask]

                # Если нашли совпадения, добавляем объем
                if len(matching_rows) > 0:
                    for structure_idx in matching_rows.index:
                        structure_work.at[structure_idx, 'temp_volume'] += id_volume
                    matched_combinations += 1

            # Переносим накопленные объемы в основной DataFrame
            pivot_df['Подтверждено_ИД'] = structure_work['temp_volume']

            self.update_progress(95, "Завершение сопоставления")

            # Статистика
            total_matched_rows = (pivot_df['Подтверждено_ИД'] > 0).sum()
            total_volume = pivot_df['Подтверждено_ИД'].sum()

            self.log(f"✓ Обработано {total_id_rows} записей ИД")
            self.log(f"✓ Найдено {matched_combinations} совпадающих комбинаций параметров")
            self.log(f"✓ Затронуто {total_matched_rows} строк в структуре")
            self.log(f"✓ Общий объем подтвержденных работ: {total_volume}")

            # Выводим примеры найденных соответствий
            ...
            # Отладочная информация если совпадений нет
            if total_matched_rows == 0:
                self.log("\nОтладочная информация:")
                self.log("Первые 3 записи ИД:")
                for idx, (_, row) in enumerate(id_work.head(3).iterrows()):
                    self.log(
                        f"  ИД {idx + 1}: Шифр='{row.get('Шифр_комплекта', '')}', Конструктивный элемент='{row.get('Конструктивный_элемент', '')}', Вид работ='{row.get('Вид_работ', '')}', Объем={row.get('Объем_по_документу', 0)}")

                self.log("\nПервые 3 записи структуры:")
                for idx, (_, row) in enumerate(structure_work.head(3).iterrows()):
                    self.log(
                        f"  Структура {idx + 1}: Проект='{row.get('Проект', '')}', Конструкция='{row.get('Конструкция', '')}', Вид работ='{row.get('Вид_работ_по_классификатору', '')}'")
